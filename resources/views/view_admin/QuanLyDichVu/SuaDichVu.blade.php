...
<head>
	...
	<link rel="stylesheet" href="{{asset('css/css_admin/css_DichVu/css_Sua.css')}}">
	...
	<div class="content">
			...
			<div class="col-md-10">
				...
				<div class="form-group">
					<form action="{{route('sua_dichvu1',['MADICHVU'=>$dichvu->MADICHVU])}}" method="POST">
						<input type="hidden" name="_token" value="{{csrf_token()}}">
						<label for="TenDichVu" id="lab_tenDichVu">Tên dịch vụ:</label>
							<input type="text" id="TenDichVu" name="TenDichVu" value="{{$dichvu->TENDICHVU}}" placeholder="Nhập tên dịch vụ" class="form-control" autocomplete="off">
						<label for="GiaTien" id="lab_giaTien">Giá tiền:</label>
							<input type="number" id="GiaTien" name="GiaTien" value="{{$dichvu->GIATIEN}}" placeholder="Nhập giá tiền" class="form-control">
						<label for="TrangThai" id="lab_trangThai">Trạng thái</label>
						<select name="TrangThai" id="TrangThai" class="form-control">
							<option value="Có"
								@if($dichvu->TRANGTHAI=="Có")
									{{"checked"}}
								@endif
							>Có</option>
							<option value="Không"
								@if($dichvu->TRANGTHAI=="Không")
									{{"checked"}}
								@endif
							>Không</option>
						</select>
